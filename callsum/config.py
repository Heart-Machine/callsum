...
def local_dir() -> Path:
    ...
    и переживать переустановку обязаны — иначе каждое обновление тянуло бы
    гигабайты заново. Поэтому %LOCALAPPDATA%, а не папка программы и не
    перемещаемый профиль.
    """
    local = os.environ.get("LOCALAPPDATA")
    return (Path(local) if local else Path.home() / "AppData" / "Local") / "callsum"
